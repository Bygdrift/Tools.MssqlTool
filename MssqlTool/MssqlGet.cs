    ...
    public partial class Mssql
    {
        ...
        public string[] GetEmptyColumns(string tableName)
        {
            ...
            var sql = "declare @tempTable TABLE(ColumnName sysname,NotNullCnt bigint);\n" +
                ...
                "SET @schemaName = '" + SchemaName + "';SET @tableName = '" + tableName + "';\n" +
        ...
        public (T First, T Last) GetFirstAndLastAscending<T>(string tableName, string columnName, bool includeNulls)
        {
            ...
            var sqlFirst = $"IF OBJECT_ID('{SchemaName}.{tableName}') IS NOT NULL BEGIN SELECT TOP(1) [{columnName}] AS Item FROM [{SchemaName}].[{tableName}] {notNull} ORDER BY [{columnName}] ASC END; ELSE BEGIN SELECT NULL END;";
            var sqlLast = $"IF OBJECT_ID('{SchemaName}.{tableName}') IS NOT NULL BEGIN SELECT TOP(1) [{columnName}] AS Item FROM [{SchemaName}].[{tableName}] {notNull} ORDER BY [{columnName}] DESC END; ELSE BEGIN SELECT NULL END;";
